...
def computeCost(X, y, theta):
  ...
  cost=0
  (m,n)=X.shape
  cost=np.sum(np.log(1+np.exp(-np.multiply(  y,np.sum(np.multiply(theta,X),axis=1)   ))),axis=0)/m
...
def computeGradient(X,y,theta):
  ...
  temp = temp=np.exp(-np.multiply(y,np.sum(np.multiply(theta,X),axis=1))).reshape(-1,1)
  grad = -np.sum(np.multiply(y.reshape(-1,1),np.multiply(X,(temp/(1+temp)))),axis=0)/X.shape[0]
